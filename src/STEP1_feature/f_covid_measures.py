...
# )
def sql_statement_01(covid_measure_indicators, start_end_date):
	cmi = covid_measure_indicators
	covidtbl = start_end_date.filter(col("window_type") == "covid") \
		.select("person_id", "measure_type",
				col("covid_length").alias("c_covid_length"),
				col("impute_covid_length").alias("c_impute_covid_length")) \
		.distinct()

	posttbl = start_end_date.filter(col("window_type") == "pos_covid") \
		.select("person_id", "measure_type",
...
def sql_statement_02(Feature_table_builder, measurement, concept):
	...
	df = ft.join(m, 'person_id', 'left') \
		...
		.agg(max(when(m.value_as_concept_id.isin([4126681, 45877985, 45884084, 9191]), 1).otherwise(0)).alias("pos_or_neg"), \
...
def sql_statement_04(covid_window, post_covid):
	...
	result = (covid_window.alias("c")
			  .join(t, ["person_id", "measure_type"], "right")
			  .groupBy("t.person_id", "t.measure_type", "t.first_pos_dt", "t.last_pos_dt")
			  .agg(min(col("c.measure_neg_date")).alias("first_neg_dt"),
				   col("'covid'").alias("window_type"))
			  .where((col("c.measure_neg_date") > col("t.first_pos_dt")) | (col("c.measure_neg_date").isNull())))
