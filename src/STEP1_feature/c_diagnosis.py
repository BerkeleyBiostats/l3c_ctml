...
def sql_statement_00(Feature_table_builder, condition_occurrence):
    ...
    joined_df = feat.join(
        ...
        (feat.person_id == co.person_id) & (
            co.condition_start_date.between(feat.pre_window_end_dt, feat.post_window_start_dt))
    ).select(
        feat["*"],
...
def sql_statement_01(Feature_Table_Builder, microvisits_to_macrovisits, pre_pre_condition, pre_condition,
                     covid_condition, post_condition):
    feat = Feature_Table_Builder
    ...
    prepretbl = (feat.join(mml, feat.person_id == mml.person_id)
                 ...
                 .withColumnRenamed('count(CASE WHEN (visit_occurrence_id IS NOT NULL) THEN visit_occurrence_id END)',
                                    'dx_count')
...
def sql_statement_02(Feature_table_builder, condition_occurrence):
    ...
    joined_df = feat.join(
        co,
        (feat.person_id == co.person_id) & (
            co.condition_start_date.between(feat.post_window_start_dt, feat.post_window_end_dt))
    ).select(
        feat["*"],
        # co.condition_concept_name,
        co.condition_concept_id,
...
def sql_statement_03(Feature_table_builder, condition_occurrence):
    ...
    joined_df = feat.join(
        co,
        (feat.person_id == co.person_id) & (
            co.condition_start_date.between(feat.pre_window_start_dt, feat.pre_window_end_dt))
    ).select(
        feat["*"],
        # co.condition_concept_name,
        co.condition_concept_id,
...
def sql_statement_04(Feature_Table_Builder, four_windows_dx_counts):
    ...
    tbl = four_windows_dx_counts.select(
        ...
        coalesce(col("pre_pre_dx_count"), col("pre_dx_count"), col("covid_dx_count"), col("post_dx_count"),
                 lit(0)).alias("pre_pre_dx_count"),
...
def sql_statement_05(Feature_table_builder, condition_occurrence):
    ...
    joined_df = feat.join(
        co,
        (feat.person_id == co.person_id) & (
            co.condition_start_date.between(feat.pre_pre_window_start_dt, feat.pre_window_start_dt))
    ).select(
        feat["*"],
        # co.condition_concept_name,
        co.condition_concept_id,
